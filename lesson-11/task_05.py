...
class Orgtech:
    ...
    def add_item(self, serial, name, model, price, count):
        "'"
        ...
        print(f"Передали на склад: Серийный номер: {serial}, Имя: {name}, Модель:{model}, "
              f"Цена:{price}, Колличество: {count} \n")
    ...
    def to_company(self, serial_number, name_company):
        ...
        try:
            for to in self.items:
                if (to == serial_number):
                    self.items.pop(serial_number)
                    # print(f'Передали со склада технику с серийным номером: {serial_number}')
...
organize_pri1.add_item('75233412', 'Epson', 'ESP-100', '400 тыс.', 1)
organize_pri1.add_item('55264152', 'HP', 'LJ', '50 тыс.', 1)
organize_pri1.add_item('55264153', 'HP', 'LJ', '50 тыс.', 1)
